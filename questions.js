//Array: Variable with quiz questions; 

var questions = [{
    title: "Inside of the HTML file, which tag defines JavaScript?",
    alternatives: ["style", "script", "!Doctype", ".js"],
    answer: "script"
},
{
    title: "Images in your webpage may have the following extensions except?",
    alternatives: [".png", ".gif", ".jpg", ".psd"],
    answer: ".psd"
},
{
    title: "How do you add a comment in a CSS file?",
    alternatives: ["/*comment*/", "//comment", "//comment//", "!--comment--"],
    answer: "/*comment*/"
},
{
    title: "Which is the correct JQuery syntax?",
    alternatives: ["$(selector).action", "selector{property:value}", "selector($).action", "variable = value"],
    answer: "$(selector).action"
},
{
    title: "Which selector does the # symbol stand for?",
    alternatives: ["class", "tag", "id", "n/a"],
    answer: "id"
}
]
